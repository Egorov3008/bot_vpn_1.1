...
async def send_welcome_message(chat_id: int, trial_status: int):
    ...
    :param trial_status: Статус пробного периода (0 - нет пробного периода, 1 - пробный период активен).
    """
    ...
    builder.row(InlineKeyboardButton(text="📢 Официальный канал", url=CHANNEL_URL), )
...
async def handle_connect_vpn(callback_query: CallbackQuery, session):
    """
    Обрабатывает нажатие кнопки "Подключить VPN" и отправляет пользователю ключ доступа.

    :param callback_query: Объект CallbackQuery, содержащий информацию о нажатой кнопке.
    :param session: Сессия для выполнения запросов к базе данных.
    """
    ...
    if "error" in trial_key_info:
        ...
        key_message = (f"🔑 <b>Ваш персональный ключ доступа:</b>\n"
                       f"<pre>{trial_key_info['key']}</pre>\n\n"
                       f"📋 <b>Быстрая инструкция по подключению:</b>\n{INSTRUCTIONS_TRIAL}")

        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="👤 Личный кабинет", callback_data="view_profile"))
        builder.row(InlineKeyboardButton(text="🍏 Скачать для iOS", url=DOWNLOAD_IOS, ),
            InlineKeyboardButton(text="🤖 Скачать для Android", url=DOWNLOAD_ANDROID, ), )

        builder.row(InlineKeyboardButton(text="🍏 Подключить на iOS", url=f'{CONNECT_IOS}{trial_key_info["key"]}', ),
            InlineKeyboardButton(text="🤖 Подключить на Android", url=f'{CONNECT_ANDROID}{trial_key_info["key"]}', ), )
...
async def handle_about_vpn(callback_query: CallbackQuery):
    """
    Обрабатывает нажатие кнопки "О нашем VPN" и отправляет информацию о VPN.
...
async def handle_back_to_menu(callback_query: CallbackQuery):
    """
    Обрабатывает нажатие кнопки "Назад" и возвращает пользователя в главное меню.
